...
df2$education_group <- factor(df2$DMDEDUC2, levels = c(1, 2, 3, 4, 5), labels = c("1", "1", 
                                   "2", "3", "4"))
# disease condition
...
# smoke condition
df2 <- df2 %>% mutate(smoke = case_when(
  SMQ020 == 2 ~ 0,
  SMQ040 == 3 & SMQ020 == 1 ~ 1,
  SMQ040 == 1 | SMQ040 == 2 ~ 2
))
df3 <- df2[complete.cases(df2[,c("is_disease", "RIDAGEYR", "BMXBMI", "education_group", "PLEVEL", "smoke")]),]
```
#age distribution
```{r}
...
df3$education_group <- factor(df3$education_group)
df3$smoke <- factor(df3$smoke)
df3$PLEVEL <- factor(df3$PLEVEL)
...
log <- glm(is_disease ~ smoke + RIDAGEYR + BMXBMI + PLEVEL + education_group, family = "binomial", data = df3.train)
summary(log)
```
